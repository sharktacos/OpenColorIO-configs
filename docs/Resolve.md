...
The working space in ACES is log, most commonly in ACEScct. This is set in the "color science" drop down and determins the processing space. Therefore when it says "process LUTs in AP1 ACEScc" in the screen shot below this actually means it will process in AP1 ACEScct, based on the color science setting. Below you can see the settings in the Preferences for ACES color management. ACEScct is the working color space. The Input is set to "no input transform" so it can be chosen for each clip as described above. The Output is set for viewing on a Rec.709 broadcast monitor.
