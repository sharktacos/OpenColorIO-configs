...
Dailies is where the camera RAW files, which will be used in finishing, are used to generate color-baked dailies and editorial media. One therefore needs a software that can properly debayer RAW camera files in a color managed ACES workflow. There are many software that can do this. For the indie filmmaker, a clear choice is [DaVinci Resolve](Resolve.md) due to the low price point. The RAW camera files would be read into Resolve using ACES color management, debayered and exported out in Rec.709 color space as h.264 for Dailies viewing, and as ProRes or DNxHD for editorial, baking the ACES Output Transform into the clips. On-set color decisions can be communicated via ASC CDLs and be passed along into ALEs. 
...
- **VFX Delivery.** VFX can deliver two types of files:
  ...
  - *High resolution EXR files to DI and Editorial Finishing.* The EXR files are returned in the same exchange format they were received: ACES2066-1 AP0. DI will ingest this into their ACES compatible color grading software as would Editorial Finishing. These can be output for delivery to multiple display mediums (film projector, broadcast television, UltraHDTV, HDR displays). Because these AP0 EXR files contain the full gamut of human vision they are also perfect for the non-graded archival master (NAM) which can be used for re-mastering into some not-yet-invented display medium.
