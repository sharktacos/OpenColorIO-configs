...
- If needed, you can easily transfer footage from Premiere Pro (````File>Export>Final Cut XML````) into Resolve (````File > Import Timeline > Import AAF, EDL, XML…````) with an XML file. To roundtrip back to Premiere, open the Edit page in Resolve, right-click the timeline from the Media Pool, and choose ````Timelines > Export > AAF/XML````, choose XML from the dropdown menu and save.
