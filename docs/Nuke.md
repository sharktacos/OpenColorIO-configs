...
What is variable, and needs to be noted in the show guide, is the [view transform](configs.md) used, which as discussed corresponds to dailies output. For clarity here we have a Nuke gizmo called [Write Dailies Sequence](https://sharktacos.github.io/VFX-software-prefs/docs/Nuke.html) that automatically creates a burn-in text on the image documenting the shot name and output transform.
