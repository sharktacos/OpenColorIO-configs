...
Below you can see the display device in parenthesis. Most are in sRGB as this is usually what you are viewing on. The equivalent to Nuke's native sRGB would be un-tone-mapped (sRGB). For details on what these view transforms do, see the [tone mapping](tonemap.md) doc. You'll find information on the gamut compression in the section below. Finally. In red you can see the display device *Rec.1886 / Rec.709 video* which is for a Rec.709 broadcast monitor with Rec.1886 gamma (2.4). You would select this if you are viewing on an HDTV (for example in 400a). It's worth noting that the standard gamma for Rec.709 for HDTV (which is used in this config) is gamma 2.4. Nuke's native Rec.709 view transform uses a non-standard gamma of approximately 1.95 which is much darker.
