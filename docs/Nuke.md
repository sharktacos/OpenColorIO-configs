...
Below you can see the display device in parethesis. Most are in sRGB as this is usually what you are viewing on. The equavalent to Nuke's native sRGB would be un-tone-mapped (sRGB). For an explanation of all of these do, see the [tone mapping](tonemap.md) doc. You'll find information on the gamut compression in the sction below. In red you can see the display device *Rec.1886 / Rec.709 video* which is for a Rec.709 broadcast monitor with Rec.1886 gamma (2.4). It's worth noting that when a client says they are working in Rec.709 they almost certainly mean gamma 2.4. Nuke's native Rec.709 has a gamma of aproximatly 1.95 which is not going to match any TV.
